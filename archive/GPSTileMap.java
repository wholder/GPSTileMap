...
/*
      add(getButton("barrel", "barrel.png",     "Marker",       "Marker"));
      add(getButton("eye",    "bullseye.png",   "Bullseye",     "Bullseye"));
...
*/
  ...
  public static class GPSMap extends JPanel {
    ...
    private static Dimension      fullMap = new Dimension(2048, 2048);
    ...
    private double                firstLat, firstLon;
    private String                tool;
    ...
    private static class MapSet implements Serializable {
      ...
      private byte[][]            imgData = new byte[3][];
      ...
      public MapSet (String name, double lat, double lon, double declination) {
        ...
        for (int ii = 0; ii < 3; ii++) {
          ...
          ulLat[ii] = pixLat[ii] - fullMap.width / 2;
          ulLon[ii] = pixLon[ii] - fullMap.height / 2;
        }
      }
      ...
      public static MapSet load (String name) throws IOException, ClassNotFoundException {
      ...
      private void writeObject (java.io.ObjectOutputStream out) throws IOException {
        for (int ii = 0; ii < maps.length; ii++) {
          ...
          ImageIO.write((BufferedImage) maps[ii], "png", bout);
          imgData[ii] = bout.toByteArray();
        }
      ...
      private void readObject(java.io.ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        maps = new Image[3];
        for (int ii = 0; ii < maps.length; ii++) {
          ByteArrayInputStream bin = new ByteArrayInputStream(imgData[ii]);
          maps[ii] = ImageIO.read(bin);
          imgData[ii] = null;
        }
      }
      
      private Image getMap (int zoom) {
        if (maps[zoom - BaseZoom] == null) {
          // Build array of image tiles
          java.util.List<BufferedImage> tmp = new ArrayList<>();
          for (int xx = 0; xx < fullMap.width; xx += imgTileSize) {
            for (int yy = 0; yy < fullMap.height; yy += imgTileSize) {
              double cLat = GPSMap.pixelYToLat(pixLat[zoom - BaseZoom] + yy + 256 - fullMap.height / 2, zoom);
              double cLon = GPSMap.pixelXToLon(pixLon[zoom - BaseZoom] + xx + 256 - fullMap.width / 2, zoom);
              try {
                String url = "http://maps.googleapis.com/maps/api/staticmap?center=" + format(cLat) + "," +
                                format(cLon) + "&zoom=" + zoom + "&size=512x512&sensor=false&maptype=satellite";
          ...
          BufferedImage mapScr = new BufferedImage(fullMap.width, fullMap.height, BufferedImage.TYPE_INT_RGB);
          ...
          maps[zoom - BaseZoom] = mapScr;
        }
        return maps[zoom - BaseZoom];
      }
    ...
    private static class MarkSet implements Serializable {
      ...
      private String getCsvCoords (Settings settings) {
        ...
        for (Waypoint way : waypoints) {
          buf.append(format(way.lat));
          buf.append(",");
          buf.append(format(way.lon));
          buf.append(",");
    ...
    abstract public static class Drawable implements Serializable {
      ...
      protected Point getMapLoc (GPSTileMap.GPSMap gpsMap) {
        int y = GPSMap.latToPixelY(lat, gpsMap.zoom) - (gpsMap.mapSet.getUlLat(gpsMap.zoom) + gpsMap.offY);
        int x = GPSMap.lonToPixelX(lon, gpsMap.zoom) - (gpsMap.mapSet.getUlLon(gpsMap.zoom) + gpsMap.offX);
    ...
    public static class GPSReference extends Marker implements Serializable {
      ...
      public int getDeltaLat () {
        return toFixed(refLat - lat);
      }
 
      public int getDeltaLon () {
        return toFixed(refLon - lon);
      }
   }
    ...
    private void setPosition (Drawable mrk, int x, int y) {
      mrk.lat = GPSMap.pixelYToLat(mapSet.getUlLat(zoom) + offY + y, zoom);
      mrk.lon = GPSMap.pixelXToLon(mapSet.getUlLon(zoom) + offX + x, zoom);
    }
        ...
        if ("arrow".equals(tool)  ||  ("pin".equals(tool) && shiftDown)  ||  ("gps".equals(tool) && shiftDown)) {
          if (shiftDown) {
            ...
            if (mrk != null  &&  mrk instanceof Waypoint) {
              ...
              Object[][] message = {{"Latitude:", lat}, {"Longitude:"}};
              ...
              if (notEmpty(latTxt)  &&  notEmpty(lonTxt)) {
                ...
                toolInfo.setText("dLat: " + format(markSet.gpsReference.refLat - markSet.gpsReference.lat) +
                               ", dLon: " + format(markSet.gpsReference.refLon - markSet.gpsReference.lon));
                repaint();
              } else if (lonTxt != null) {
          ...
          if (mapSet != null) {
            ...
            toolInfo.setText(format(lat) + ", " + format(lon));
          } else {
          ...
          if (mapSet != null) {
            double lat = GPSMap.pixelYToLat(mapSet.getUlLat(zoom) + offY + my, zoom);
            double lon = GPSMap.pixelXToLon(mapSet.getUlLon(zoom) + offX + mx, zoom);
            markSet.waypoints.add(new Waypoint(lat, lon, settings.getDefault()));
            toolInfo.setText(format(lat) + ", " + format(lon));
            repaint();
          } else {
            toolInfo.setText("Map not loaded");
          ...
          if (mapSet != null) {
            double lat = GPSMap.pixelYToLat(mapSet.getUlLat(zoom) + offY + my, zoom);
            double lon = GPSMap.pixelXToLon(mapSet.getUlLon(zoom) + offX + mx, zoom);
            markSet.gpsReference = new GPSReference(lat, lon);
            repaint();
          } else {
            toolInfo.setText("Map not loaded");
          }
          ...
          if (mapSet != null) {
            ...
            if (mkr instanceof Waypoint) {
              ...
              toolInfo.setText("Not deletable");
            } else {
          ...
          if (mapSet != null) {
            if (tapePressed) {
              double lat2 = GPSMap.degreesToRadians(GPSMap.pixelYToLat(mapSet.getUlLat(zoom) + offY + event.getY(), zoom));
              double lon2 = GPSMap.degreesToRadians(GPSMap.pixelXToLon(mapSet.getUlLon(zoom) + offX + event.getX(), zoom));
              // Note: Radius of Earh in kilometers is 6371
              double dist = Math.acos(Math.sin(firstLat) * Math.sin(lat2) + Math.cos(firstLat) * Math.cos(lat2) * Math.cos(lon2 - firstLon)) * 6371;
              // Note: 1 kilometer is 3280.84 feet
              toolInfo.setText("Distance is " + feetFmt.format(dist * 3280.84) + " feet");
              tapePressed = false;
            } else {
              firstLat = GPSMap.degreesToRadians(GPSMap.pixelYToLat(mapSet.getUlLat(zoom) + offY + event.getY(), zoom));
              firstLon = GPSMap.degreesToRadians(GPSMap.pixelXToLon(mapSet.getUlLon(zoom) + offX + event.getX(), zoom));
              toolInfo.setText("Select 2nd point");
              tapePressed = true;
            }
            // Note: compute inches/pixel
            //  Zoom 19 distance is 8.985435874169168 inches/pixel
            //  Zoom 20 distance is 4.492912271401151 inches/pixel
            //  Zoom 21 distance is 2.2463006662281613 inches/pixel
            /*
            for (int zx = 19; zx <= 21; zx++) {
              double lat1 = GPSMap.degreesToRadians(GPSMap.pixelYToLat(mapSet.getUlLat(zx) + offY + 0, zx));
              double lon1 = GPSMap.degreesToRadians(GPSMap.pixelXToLon(mapSet.getUlLon(zx) + offX + 0, zx));
              double lat2 = GPSMap.degreesToRadians(GPSMap.pixelYToLat(mapSet.getUlLat(zx) + offY + 0, zx));
              double lon2 = GPSMap.degreesToRadians(GPSMap.pixelXToLon(mapSet.getUlLon(zx) + offX + 100, zx));
              double dist = Math.acos(Math.sin(lat1) * Math.sin(lat2) + Math.cos(lat1) * Math.cos(lat2) * Math.cos(lon2 - lon1)) * 6371;
              System.out.println("Zoom " + zx + " distance is " + (dist * 3280.84 * 0.12) + " inches");
            }
            */
          } else {
            toolInfo.setText("Map not loaded");
    ...
    class MyMouseMotionAdapter extends MouseMotionAdapter  {
      public void mouseDragged (MouseEvent event) {
        ...
        if ("arrow".equals(tool)) {
          ...
          offX = Math.max(0, Math.min(fullMap.width - win.width, pX + dX));
          offY = Math.max(0, Math.min(fullMap.height - win.height, pY + dY));
          repaint();
        }
      }
    }
    
    public GPSMap (GPSTileMap gpsTileMap, Preferences prefs, JLabel toolInfo) {
  ...
  public void loadMap (String mapName) throws Exception {
      setMap(GPSMap.MapSet.load(mapName));
    }
    ...
    public void setMap (MapSet mapSet) {
      ...
      initSettiings();
      showMarkers = prefs.getBoolean("markers.on", false);
      ...
      showWayLines = prefs.getBoolean("waylines.on", false);
      repaint();
    }
    ...
    public String[] getUploadData () {
      ...
      if (markSet.gpsReference != null) {
        ...
        buf.append(toHexByte(toFixed(markSet.gpsReference.lat), 8));
        buf.append(toHexByte(toFixed(markSet.gpsReference.lon), 8));
      ...
      for (GPSMap.Waypoint way : markSet.waypoints) {
        check = 0;
        int lat = toFixed(way.lat);
        int lon = toFixed(way.lon);
        buf = new StringBuilder("$");
      ...
      lines.add("!\n\r");
      String[] tmp = lines.toArray(new String[lines.size()]);
      if (false) {
        for (String aTmp : tmp) {
  ...
  private class Uploader {
    ...
    private Uploader (final String[] lines, final JMenu menu, JLabel toolInfo) {
      try {
        toolInfo.setText("Uploading...");
        jsscPort.openPort();
  ...
  GPSTileMap () {
    ...
    JLabel toolInfo = new JLabel("Info:");
    JPanel topPane = new JPanel(new GridLayout(0,2));
    topPane.add(toolBar);
    topPane.add(toolInfo);
    ...
    for (int ii = GPSMap.BaseZoom; ii <= GPSMap.MaxZoom; ii++) {
      ...
      item.addItemListener(e -> {
        ...
        if (item1.isSelected()) {
          gpsMap.zoom = Integer.parseInt(item1.getText());
          prefs.putInt("window.zoom", gpsMap.zoom);
          gpsMap.repaint();
        }
      });
    }
    ...
    mItem5.addActionListener(ev -> {
      ...
      for (GPSMap.Waypoint way : gpsMap.markSet.waypoints) {
        if (lastWay != null) {
            double lat1 = GPSMap.degreesToRadians(lastWay.lat);
            double lon1 = GPSMap.degreesToRadians(lastWay.lon);
            double lat2 = GPSMap.degreesToRadians(way.lat);
            double lon2 = GPSMap.degreesToRadians(way.lon);
            double dist = Math.acos(Math.sin(lat1) * Math.sin(lat2) + Math.cos(lat1) * Math.cos(lat2) * Math.cos(lon2 - lon1))* 6371;
    ...
    mItem7.addActionListener(ev -> gpsMap.resetSettings());
